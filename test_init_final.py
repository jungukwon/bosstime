...
while True:
	...
	async def noticeDel_(ctx):
		if ctx.message.channel.id == basicSetting[7]:
			...
			await ctx.send( '< 파티 삭제완료 >', tts=False)
		else:
			return	
	...
	async def notice2Del_(ctx):
		if ctx.message.channel.id == basicSetting[7]:
			...
			await ctx.send( '< 파티2 삭제완료 >', tts=False)
		else:
			return
	...
	async def notice3Del_(ctx):
		if ctx.message.channel.id == basicSetting[7]:
			...
			await ctx.send( '< 대기자 삭제완료 >', tts=False)
		else:
			return
	...
	async def race_(ctx, *, msg: str):
		if ctx.message.channel.id == basicSetting[7] or ctx.message.channel.id == basicSetting[19]:
			...
			random_pos = []
			racing_result = []
			output = ':camera: :camera: :camera: 동물 레이스! :camera: :camera: :camera:\n'
			...
			if len(racing_member) == 1:
				...
				for j in range(len(random_pos[0])):
					if j%2 == 0:
						output =  ':camera: :camera_with_flash: :camera: 동물 레이스! :camera_with_flash: :camera: :camera_with_flash:\n'
					else :
						output =  ':camera_with_flash: :camera: :camera_with_flash: 동물 레이스! :camera: :camera_with_flash: :camera:\n'
					str_racing_field = []
					for i in range(len(racing_member)):
						temp_pos = cur_pos[i]
