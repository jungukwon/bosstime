	...
	async def on_message(msg):
		...
		if client.get_channel(channel) != msg.channel:
			...
			if message.content == command[0]:
				...
				command_list += command[22] + ' [파티원]\n'     #파티2
				command_list += command[22] + '삭제\n'     #파티2
				command_list += command[23] + '\n'     #대기
				command_list += command[23] + ' [파티원]\n'     #대기
			...
			if message.content == command[18] + '삭제':
				...
				await client.get_channel(channel).send( '< 파티 삭제완료 >', tts=False)
			...
			if message.content == command[22] + '삭제':
				...
				await client.get_channel(channel).send( '< 파티2 삭제완료 >', tts=False)
			...
			if message.content.startswith(command[23] + ' '):
				...
				await client.get_channel(channel).send( '< 대기 등록완료 >', tts=False)
			
			if message.content == command[23] + '삭제':
				contents = repo.get_contents("notice3.ini")
				repo.update_file(contents.path, "notice3 삭제", '', contents.sha)
				await client.get_channel(channel).send( '< 대기 삭제완료 >', tts=False)
